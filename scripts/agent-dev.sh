...
# "pshelf-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "pshelf-dev" is overwritten by
...
PROJECT_NAME="pshelf"
AGENT_NAME="pshelf-dev"
WORKSPACE="/workspaces/pshelf"
...
if [ -x "/workspaces/pshelf/scripts/agent-dev.sh" ]; then
  "/workspaces/pshelf/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
